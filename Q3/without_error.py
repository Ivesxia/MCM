...
# 提取核心数据
patient_id=df['孕妇编号'].values
height_data_whole = df['身高'].values
weight_data_whole = df['体重'].values
age_data_whole = df['孕妇年龄'].values
Y_data_whole = df['Y染色体浓度'].values

grouped = df.groupby('孕妇编号')

for patient_id, group in grouped:
    height_data = group['身高'].values
    weight_data = group['体重'].values
    age_data = group['孕妇年龄'].values
    ...
    print()
# 如果你想要将数据进一步处理，比如返回一个字典
patient_data = {}
for patient_id, group in grouped:
    patient_data[patient_id] = {
        'Height': group['身高'].values,
        'Weight': group['体重'].values,
        'Age': group['孕妇年龄'].values
    }
